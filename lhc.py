...
group.add_argument("-L", "--list", action='store_true', help="list PVC information")
group.add_argument("-R", "--remove", action='store_true', help="remove PVC snapshots")
...
parser.add_argument("-v", "--verbose", action='store_true', help="show snapshot detail when listing and verbose info when removing")

require_url = None is os.environ.get('LONGHORN_URL', None)
parser.add_argument("-u", "--url", default=os.environ.get('LONGHORN_URL'), help="longhorn API URL (default: environment value of 'LONGHORN_URL')", required=require_url)
...
for v in volumes_full.data:
  ...
  volumes_ns_pvc[v.kubernetesStatus['namespace']][v.kubernetesStatus['pvcName']] = v
